...
ZTEST(driver_tests, test_display_initialization)
{
    ...
    LOG_INF("Display initialization test passed - %dx%d resolution", 
...
ZTEST(driver_tests, test_display_update_regions)
{
    ...
    zassert_equal(mock_display.last_update_w, SHARP_LCD_WIDTH, "Full update width should match display");
    zassert_equal(mock_display.last_update_h, SHARP_LCD_HEIGHT, "Full update height should match display");
    ...
    zassert_equal(mock_display.spi_transaction_count, 3, "Should have performed 3 SPI transactions");
...
ZTEST(driver_tests, test_display_performance)
{
    ...
    LOG_INF("16 small updates took %d ms (simulated), total SPI transactions: %zu", 
            duration_ms, mock_display.spi_transaction_count);
