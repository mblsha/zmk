...
ZTEST(driver_tests, test_trackpad_initialization)
{
    ...
    zassert_true(device_is_ready(trackpad_dev), "BlackBerry trackpad should initialize successfully");
...
ZTEST(driver_tests, test_trackpad_motion_detection)
{
    ...
    zassert_equal(mock_responses[0].cmd, BB_TP_CMD_READ_MOTION, 
                  "Motion command should be correct");
