...
ZTEST(driver_tests, test_drv2605_initialization)
{
    ...
    assert_register_value(DRV2605_MODE, DRV2605_MODE_INTERNAL_TRIGGER, "Initial mode should be internal trigger");
    assert_register_value(DRV2605_LIBRARY_SELECTION, DRV2605_LIBRARY_LRA, "Initial library should be LRA");
...
ZTEST(driver_tests, test_drv2605_sequence_playback)
{
    ...
    zassert_equal(mock_drv2605_state.waveform_seq[0], WAVEFORM_CLICK, "First waveform should be click");
    zassert_equal(mock_drv2605_state.waveform_seq[1], WAVEFORM_TICK, "Second waveform should be tick");
    zassert_equal(mock_drv2605_state.waveform_seq[2], WAVEFORM_BUZZ, "Third waveform should be buzz");
...
ZTEST(driver_tests, test_drv2605_auto_calibration)
{
    ...
    assert_register_value(DRV2605_MODE, DRV2605_MODE_AUTO_CALIBRATION, "Mode should be set to auto-calibration");
    zassert_true(mock_drv2605_state.calibration_done, "Calibration should complete successfully");
    zassert_equal(mock_drv2605_state.auto_cal_result, 0xA0, "Calibration result should be in valid range");
    zassert_equal(mock_drv2605_state.auto_cal_back_emf, 0x6F, "Back-EMF result should be in valid range");
...
ZTEST(driver_tests, test_drv2605_power_management)
{
    ...
    zassert_false(mock_drv2605_state.device_enabled, "Device should be disabled when enable pin is low");
    
    mock_drv2605_state.device_enabled = true;   /* Simulate GPIO enable pin high */
    zassert_true(mock_drv2605_state.device_enabled, "Device should be enabled when enable pin is high");
...
ZTEST(driver_tests, test_drv2605_error_conditions)
{
    ...
    zassert_true((mock_drv2605_state.status & 0x02) != 0, "Overcurrent detection should be flagged");
    
    /* Test overtemperature condition */
    mock_drv2605_state.status |= 0x08; /* Set OVER_TEMP bit */
    zassert_true((mock_drv2605_state.status & 0x08) != 0, "Overtemperature condition should be flagged");
...
ZTEST(driver_tests, test_drv2605_library_selection)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(test_libraries); i++) {
        ...
        assert_register_value(DRV2605_LIBRARY_SELECTION, test_libraries[i], "Library selection should be correct");
